# --- START OF FILE telegram_integration.py ---
import asyncio
import json
import logging
import os
...
class TelegramIntegration:
                    ...
                    for conversation_id, conversation in list(conversations.items()):
                        ...
                        try:
                            ...
                            metadata = {  # todo: добавить в каждое сообщение имя партнера, добавить имя сделки
                                ...
                                "date": date_str,
                                "author_username": conversation.get("author", {}).get(
                                    "username") if conversation else None,
                                "author_first_name": conversation.get("author", {}).get(
                                    "first_name") if conversation else None,
                                "deal_id": conversation["deal_id"] if conversation.get("deal_id") else {},
                                "deal_title": conversation["deal_title"] if conversation.get("deal_id") else {},
                                "company_id": conversation["company_id"] if conversation.get("deal_id") else {},
                                "company_title": conversation["company_title"] if conversation.get("deal_id") else {}
                            }
                            collection.upsert(
            ...
            if updates:
                for update in updates:
                    ...
                    if "message" in update:
                        ...
                        logging.info(f"Получено сообщение из чата {chat_title} от {date}")  # Removed the message text
    ...
    async def stop(self):
        ...
        if self.periodic_save_task:
            self.periodic_save_task.cancel()  # Stop periodic saving
    ...
    async def start(self):
        ...
        asyncio.create_task(self.telegram_message_collector())
